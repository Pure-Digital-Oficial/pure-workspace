...
import { DeleteCategoryTransactionDto } from '@pure-workspace/domain';
...
export class DeleteCategoryTransactionService {
  ...
  delete(
    deleteCategoryTransactionDto: Omit<
    ...
    return this.session.findSession().pipe(
      switchMap((session) => {
        const { id } = deleteCategoryTransactionDto;
        if (!session.id) {
          throw new Error('User ID not available');
        }

        return this.httpClient.delete<{ category_transaction_id: string }>(
          this.apiUrl + `/transaction/delete-category-transaction/${id}`,
          {
      ...
      catchError((error) => {
        const errorText = 'Erro ao deletar a categoria da transação';
